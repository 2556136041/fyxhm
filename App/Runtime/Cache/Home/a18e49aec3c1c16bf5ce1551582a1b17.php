...
<body>
	...
	<section id="secondary_bar">
		...
		<div class="breadcrumbs_container">
			<article class="breadcrumbs"><a href="/admin.html">后台管理系统</a> <div class="breadcrumb_divider"></div> <a class="current">修改产品</a></article>
		</div>
...
<section id="main" class="column">
    <form id="form1" name="form1" method="post" action="/updatepro.html">
	<article class="module width_full">
		<header><h3>修改产品</h3></header>
			   <div class="module_content">
			   <fieldset>
						<label>位置</label>
                            封面<input name="choice" type="radio" value="1"  style="width: 20px;height: 20px;vertical-align:middle;color:blue;">
                            正常<input name="choice" type="radio" value="0" style="width: 20px;height: 20px;vertical-align:middle;">                               
				</fieldset>
				<script>
                    $("input[name='choice'][value='<?php echo ($data["choice"]); ?>']").attr("checked","checked");

			    </script>
				<fieldset>
						<label>型号</label>
							<input style="height:40px;font-size:18px;" type="text" name="prono" value="<?php echo ($data["prono"]); ?>">
				</fieldset>
                <fieldset>
						<label>名称</label>
							<input style="height:40px;font-size:18px;" type="text" name="proname" value="<?php echo ($data["proname"]); ?>">
				</fieldset>
                <fieldset>
						<label>价格</label>
							<input style="height:40px;font-size:18px;" type="text" name="proprice" value="<?php echo ($data["proprice"]); ?>">
				</fieldset>
                <fieldset>
						<label>描述</label>
						<textarea style="padding:8px 0px 0px 8px;margin-top:8px;font-size:18px;" rows="8" name="proabout" id="lead"><?php echo ($data["proabout"]); ?></textarea>
				</fieldset>
				
             </div>
		<footer>
			<div class="submit_link">
				<input type="hidden" name="id" value="<?php echo ($data["proid"]); ?>">
                <input type="hidden" name="propic" value="<?php echo ($data["propic"]); ?>">
