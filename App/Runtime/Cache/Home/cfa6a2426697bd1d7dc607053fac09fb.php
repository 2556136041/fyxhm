...
<body>
	...
	<section id="secondary_bar">
		...
		<div class="breadcrumbs_container">
			<article class="breadcrumbs"><a href="/admin">后台管理中心</a> <div class="breadcrumb_divider"></div> <a class="current">管理用户</a></article>
		</div>
			...
			<div id="tab1" class="tab_content">
				<table class="tablesorter" cellspacing="0" style="margin:0"> 
					...
					<?php if(is_array($data)): foreach($data as $key=>$value): ?><tr>
			    				...
			    				<td>
                                <?php if($value["class"] == 1): ?>管理员
                                <?php else: ?>
                                一般用户<?php endif; ?>
                                </td> 
			    				<td>
			    				<?php if($flag == true): ?><input type="image" src="/Public/images/comm/icn_edit.png" title="Edit" onClick="window.location.href='/userupdate/id/<?php echo ($value["userid"]); ?>'">
                                <?php else: ?>
			    				<input type="image" src="/Public/images/comm/icn_edit.png" title="Edit" disabled="disabled" onClick="window.location.href='/userupdate/id/<?php echo ($value["userid"]); ?>'"><?php endif; ?>
			    				<?php if($flag == true): ?><input type="image" src="/Public/images/comm/icn_trash.png" title="Trash" onClick="window.location.href='/userdel/id/<?php echo ($value["userid"]); ?>'">
				    			<?php elseif($flag == 0): ?>
				    			<!--
				    				<input type="image" src="/Public/images/comm/icn_trash.png" title="Trash" disabled="disabled" onClick="window.location.href='/userdel/id/<?php echo ($value["userid"]); ?>'">
								-->
				    			<?php else: endif; ?>
			    				</td>
